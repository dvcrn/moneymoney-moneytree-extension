...
class Main {
	...
	static function ListAccounts(knownAccounts) {
		...
		Storage.set("get_accounts_response_" + current_hour, accountsResponse);
	...
	static function RefreshAccount(account:{
		...
		var current_hour = Date.now().getHours();
		var cache_key = "get_accounts_response_" + current_hour;

		var getAccountsResponse:Array<MTAccount> = Storage.get(cache_key);

		if (getAccountsResponse == null) {
			getAccountsResponse = Moneytree.getAccounts(accessToken);
			Storage.set(cache_key, getAccountsResponse);
		}
